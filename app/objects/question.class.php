...
class question
{
	...
	function __construct($req_ID, $req_Text, $req_Section, $req_Added, $req_Notes) {
		$this->ID = $req_ID;
		$this->Text = $req_Text;
	...
	public function get_WFTDA_Link() {
		...
		if ($this->get_Section() && $this->is_taxonomy_and_value("rule-set", "WFTDA7")) {
			...
			if (preg_match("@^([1-9][\.]?)+@", $this->get_Section())) {
				...
				if (count($section_array) == 1) {
					return "http://wftda.org/rules/20141201/section/" . $section_array[0];
				} elseif (count($section_array) <= 3) {
					return "http://wftda.org/rules/20141201/section/" . $section_array[0] . "." . $section_array[1];
				} else {
					return "http://wftda.org/rules/20141201/section/" . $section_array[0] . "." . $section_array[1] . "." . $section_array[2];
				}
			}
		}
		...
		if ($this->get_Section() && $this->is_taxonomy_and_value("rule-set", "WFTDA6")) {
			...
			if (preg_match("@^([1-9][\.]?)+@", $this->get_Section())) {
				...
				$section_array = explode(".", $this->get_Section());
				if (count($section_array) == 1) {
					return "http://wftda.org/rules/20130615/section/" . $section_array[0];
				} elseif (count($section_array) <= 3) {
					return "http://wftda.org/rules/20130615/section/" . $section_array[0] . "." . $section_array[1];
				} else {
					return "http://wftda.org/rules/20130615/section/" . $section_array[0] . "." . $section_array[1] . "." . $section_array[2];
				}
			}
		}
	...
	public function is_answers_different($req_new_answers) {
		...
		foreach ($req_new_answers as $new_answer) {
			...
			foreach ($existing_answers as $existing_answer) {
				if (($existing_answer->is_correct() == $new_answer->is_correct()) && ($existing_answer->get_Text() == $new_answer->get_Text()) && ($existing_answer->get_Question_ID() == $new_answer->get_Question_ID())) {
